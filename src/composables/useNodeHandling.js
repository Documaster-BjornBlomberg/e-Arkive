...
export function useNodeHandling() {
  const { loading: nodeLoading, error: nodeError, getRootNodes, getChildNodes, getNodeById, getFilesByNodeId, createNode, updateNode, deleteNode } = useGraphQL();
  const rootNodes = ref([]);
  const expandedNodeIds = reactive(new Set());
  const loadedChildrenMap = reactive(new Map());
  const isNodePanelOpen = ref(true);
  const selectedNodeId = ref('1'); // Default to root node (ID: 1)
  const nodeFiles = ref([]);
  const currentNode = ref(null);
  const allNodesFlat = ref([]); // For upload dropdown
  ...
  const fetchNodeFiles = async (nodeId) => {
    try {
      nodeFiles.value = await getFilesByNodeId(nodeId);
      return nodeFiles.value;
    } catch (error) {
  ...
  const toggleNodeExpand = async (nodeId) => {
    if (expandedNodeIds.has(nodeId)) {
      ...
      // Ensure the component re-renders by potentially updating the rootNodes ref if needed,
      // although modifying the nested object should trigger reactivity.
      // Force update if necessary: rootNodes.value = [...rootNodes.value];
      return children;
    }
  };
  ...
  const addNewNode = async (nodeName, parentId) => {
    try {
  ...
  const fetchAllNodesRecursiveHelper = async (nodeId, depth = 0, flatList = []) => {
    try {
  ...
  const getAllNodesForUpload = async () => {
    allNodesFlat.value = []; // Clear previous list
    try {
      // Start recursion from the root node(s)
      // Assuming a single conceptual root or multiple top-level nodes
  ...
  return {
    ...
    removeNode,
    initialize,
    getAllNodesForUpload // Export the new function
  };
}
